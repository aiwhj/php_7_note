 ...
 */
 /**
  * 与处理程序提供了条件预编译的功能。可以按不同的条件去编译不同的程序部分，因而查收呢灰姑娘不同的目标代码文件。
  * 这对于程序的移植和调试是很有用的。条件编译有三种形式，分别是：
  * 1.#ifdef 标识符 ... #else ... #endif
  * 它的功能是，如果标识符已被#define命令定义过，则对对陈续断1进行编译；否则对程序2进行编译。如果没有程序2（它为空）。
  * 本格式的#else可以没有，即可写为：
  * #ifdef 标识符 ... #endif
  * 2.#ifndef 标识符 ... #else ... #endif
  * 与第一种形式的区别是将"ifdef"改为"ifndef"。它的功能是，如果标识符被#define命令定义过则对程序段1进行编译，否则对程序段2惊醒编译。
  * 3.#if 常量表达式 ... #else ... #endif。它的功能是，如果常量表达式为真（非0），则对程序段1进行编译，否则对程序段2进行编译。
  */
 ...
 * artc是命令行总的参数个数。
 * argv[]是argc多个参数，其中第0个参数是程序的全名，以后的参数命令行后面跟的用户输入的参数
 * char *argv[]是一个指向字符的数组，其大小是int argc，主要用于命令行参数argv[]，数组里每个元素代表一个参数
 * 
 * int main()，许多main函数时不待参数的。因此main的括号都是空括号。实际上，main函数可以带参数，这个
 * ,这个参数可以认为是main函数的形式参数。C语言规定main函数的参数只能有两个，习惯上这两个参数写为argc和argv
 * 。因此，main函数的函数头可写为main(argc, argv)。C语言还规定argc(第一个形参)必须是整形变量，argv
 *（第二个形参）必须是指向字符串的指针数组。加上形参说明后，main函数的函数应写为：
 * mian(argc, argv);
 * int argc;
 * char *argv[];
 * 或者
 ...
 *
 * 由于main函数不能被其他函数调用，因此不可能在程序内部取得实际值。name何处把实参值赋值给main函数
 * 的形参呢？实际上，main函数的参数是从操作系统命令行上获得的。当我们运行一个可执行文件是，在提示符
 * 下键入文件名，再输入实际参数即可把这行参数传送到main的形参中去。
 */
int main(int argc, char *argv[])
#endif
{ 
/**
 * 由于要定义许多产量，在我们考虑定义某个常量时，就必须返回检查原来此常量是否定义，if defined宏就是用来解决这的。
 * #if defined a 
 * #undef a
 * #define a 200
 * #endif
...
# else
	...
	//TODO:wchar_t没有找到定义
	wchar_t **argv_wide;
	//char **：表示字符串数组。例如 char **args={"abc","china", "System"};
	char **argv_save = argv;
	//typedef int BOOL
...
# endif
	...
	int c;
	int exit_status = SUCCESS;//#define SUCCESS 0
	...
	sapi_module_struct *sapi_module = &cli_sapi_module;//cli_sapi_module是一个静态变量

	/*
	 * Do not move this initialization. It needs to happen before argv is used
	 * in any way.
	 */
	 //argv = {"/usr/bin/php", "1.php", 0x0}
	...
	//注意：additional_functions是一个数组。
	//TODO:cli_sapi_module.additional_functions是在哪里声明的？
...
exit_loop:
	...
	if (sapi_module == &cli_sapi_module) {
		if (ini_entries) {
			 ...
			 * 由src指向地址为其实地址的连续n个字节的数据复制到以dest指向地址为其实地址的空间内
			 */
